...
import { headShakeAnimate } from '../../constants/helperStyles';

export const menuSwitchStyles = (theme, open) => css`
  position: fixed;
  z-index: 2000;
  width: 48px;
  height: 48px;
...
export const menuStyles = (theme) => css`
  ...
  & .menu-social {
    ...
    & li {
      ...
      & a:hover .instagram-text,
      & a:focus .instagram-text {
      ...
      &.email a:hover .social-icon,
      &.email a:focus .social-icon {
        background: ${theme.colors.google};
        color: ${theme.colors.white};
      }
      &.email a:hover .social-text,
      &.email a:focus .social-text {
        background: linear-gradient(90deg, rgba(66,133,244,1) 37%, rgba(187,0,27,1) 37%, rgba(66,133,244,1) 37%, rgba(234,67,53,1) 60%, rgba(251,188,5,1) 74%, rgba(52,168,83,1) 85%);
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        color: ${theme.colors.white};
      }
